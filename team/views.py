...
from .serializers import (
    TeamAddRemoveEmployeesSerializer,
    TeamListSerializer,
    TeamSerializer,
)
    ...
    def add_employees(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            if new_employees:
                ...
                emp = ", ".join(
                    [employee.get_full_name() for employee in new_employees]
                )
                return Response({"message": f"{emp} added to the team successfully"})
            else:
                return Response(
                    {"message": "All specified employees are already in the team."}
                )
    ...
    def remove_employees(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            if employees_to_remove:
                ...
                emp = ", ".join(
                    [employee.get_full_name() for employee in employees_to_remove]
                )
                return Response(
                    {"message": f"{emp} removed from the team successfully"}
                )

            return Response(
                {"message": "No specified employees were found in the team."}
            )
